...
export const decodeErrorLocation = (type: HL7Type, settings: HL7Settings) =>
  arrayWithContext('ErrorLocation (ERL)', type, settings, (data: unknown[]) => ({
    ...
    SegmentSequence: reqComponent('ERL.2', 0, float, data[2]),
    FieldPosition: optComponent('ERL.3', 0, float, data[3]),
    FieldRepetition: optComponent('ERL.4', 0, float, data[4]),
    ComponentNumber: optComponent('ERL.5', 0, float, data[5]),
    SubComponentNumber: optComponent('ERL.6', 0, float, data[6]),
  }));
